  ...
  return (
    <div className="min-h-screen bg-[#D6E6D6] flex items-center justify-center">
      <div className="bg-white rounded-2xl border border-[#C8D8C8] p-8 w-full max-w-sm">
        ...
        {!showForm ? (
          <>
            <h1 className="text-xl font-semibold text-gray-900 tracking-tight mb-1">Bienvenue</h1>
            <p className="text-sm text-gray-400 mb-8">SUP-PHOTO · Espace de suivi</p>

            <div className="flex flex-col gap-3">
              <button
                onClick={() => setShowForm(true)}
                className="w-full bg-[#5C7A5C] text-white py-2.5 rounded-lg text-sm font-medium hover:bg-[#4A6A4A] transition-colors"
              >
                Se connecter
              </button>
              <Link
                href="/inscription"
                className="w-full border border-[#C8D8C8] text-gray-700 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors text-center"
              >
                Créer mon espace
              </Link>
            </div>
          ...
          <>
            ...
            <p className="text-xs text-gray-400 text-center mt-4">
              <button onClick={() => setShowForm(false)} className="text-[#5C7A5C] hover:underline">
                ← Retour
              </button>
            </p>
          </>

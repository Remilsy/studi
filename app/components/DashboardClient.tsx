...
const origineConfig: Record<string, { label: string; color: string; bg: string }> = {
  interne:  { label: 'Interne',  color: '#059669', bg: 'rgba(5,150,105,0.1)'   },
  externe:  { label: 'Externe',  color: '#7C3AED', bg: 'rgba(124,58,237,0.1)'  },
}
...
function exportCSV(etudiants: Etudiant[]) {
  const headers = ['Prénom', 'Nom', 'Email', 'Statut', 'Section', 'Formation', 'Origine', 'Candidatures', 'Progression']
  ...
  a.href = url; a.download = `etudiants_${new Date().toISOString().slice(0, 10)}.csv`
...
export default function DashboardClient({ etudiants }: { etudiants: Etudiant[] }) {
  ...
  const hasOrigine = useMemo(() => etudiants.some(e => e.origine), [etudiants])
  ...
  const filtered = useMemo(() => etudiants.filter(e => {
    ...
    if (filterOrigine   && e.origine       !== filterOrigine)   return false
    return true
  }), [etudiants, search, filterStatut, filterNiveau, filterFormation, filterOrigine])
  ...
  return (
    <div style={{
      ...
      border: '1px solid var(--dash-card-border)', borderRadius: '24px',
      ...
      <div className="p-4 flex flex-wrap items-center gap-3" style={{ borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
        <input
          ...
          style={{ ...ctrl, flex: '1 1 180px', minWidth: '180px' }}
        />
        <select value={filterStatut} onChange={e => setFilterStatut(e.target.value)} style={ctrl}>
          <option value="">Tous les statuts</option>
          <option value="en_preparation">En préparation</option>
          <option value="en_recherche">En recherche</option>
          <option value="place">Placé</option>
        </select>
        <select value={filterNiveau} onChange={e => setFilterNiveau(e.target.value)} style={ctrl}>
          <option value="">Toutes sections</option>
          {niveaux.map(n => <option key={n} value={n}>{niveauLabels[n] ?? n}</option>)}
        </select>
        <select value={filterFormation} onChange={e => setFilterFormation(e.target.value)} style={ctrl}>
          <option value="">Toutes formations</option>
          <option value="alternance">Alternance</option>
          <option value="initial">Initial</option>
        </select>
        {hasOrigine && (
          <select value={filterOrigine} onChange={e => setFilterOrigine(e.target.value)} style={ctrl}>
            <option value="">Interne & externe</option>
            <option value="interne">Interne</option>
            <option value="externe">Externe</option>
          </select>
        )}
        <button onClick={() => exportCSV(filtered)} style={{ ...ctrl, color: '#6B7280', cursor: 'pointer' }}>↓ CSV</button>
        <span className="text-xs text-gray-400 shrink-0">{filtered.length} étudiant{filtered.length > 1 ? 's' : ''}</span>
      </div>

      {filtered.length === 0 ? (
        <div className="p-10 text-center"><p className="text-sm text-gray-300 italic">Aucun étudiant ne correspond à ta recherche.</p></div>
      ) : (
        <div>
          {filtered.map((e, idx) => {
            const s = statutConfig[e.statut] || { label: e.statut, color: '#9CA3AF', bg: 'rgba(156,163,175,0.1)', bar: '#D1D5DB' }
            const o = e.origine ? origineConfig[e.origine] : null
            const urgence = e.statut === 'en_recherche' && (e.nb_candidatures ?? 0) === 0
            const niveauLabel = niveauLabels[e.niveau] ?? e.niveau
            return (
              ...
              >
                ...
                <div className="w-9 h-9 rounded-2xl flex items-center justify-center text-xs font-bold shrink-0" style={{ background: s.bg, color: s.color }}>
                  {e.prenom[0]}{e.nom[0]}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {urgence && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full shrink-0" style={{ background: 'rgba(249,115,22,0.1)', color: '#EA580C' }}>À relancer</span>}
                    {o && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full shrink-0" style={{ background: o.bg, color: o.color }}>{o.label}</span>}
                  </div>
                  <p className="text-xs text-gray-400 truncate">{e.email}</p>
                </div>

                <div className="hidden sm:flex items-center gap-2 shrink-0">
                  <span className="text-xs text-gray-400">{niveauLabel}</span>
                  <span className="text-gray-200">·</span>
                  <span className="text-xs text-gray-400">{e.type_formation === 'alternance' ? 'Alternance' : 'Initial'}</span>
                </div>

                <span className="hidden md:inline-flex text-[10px] font-bold px-2.5 py-0.5 rounded-full shrink-0" style={{ background: s.bg, color: s.color }}>{s.label}</span>
                ...
                <div className="hidden lg:flex items-center gap-2 w-24 shrink-0">
                  <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(0,0,0,0.07)' }}>
                    <div className="h-full rounded-full transition-all" style={{ width: `${e.score_progression ?? 0}%`, background: `linear-gradient(90deg, ${s.bar}, ${s.bar}99)` }}/>
                  </div>
